...
Better-Auth database tables are created automatically on first run — no manual migration needed.
...
| `npm run db:better-auth:migrate` | Manually run Better-Auth migrations (useful before running `db:codegen` to ensure tables are up to date) |
| `npm run db:codegen` | Regenerate `src/types/database.types.ts` from the live database schema — **do not edit this file manually** |
